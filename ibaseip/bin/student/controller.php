...
function doCreateMessage(){
	global $mydb;
		if(isset($_POST['save'])){
			...
			$student = $_SESSION['FNAME'] . ' ' . $_SESSION['LNAME'];
			$IDNO = $_SESSION['IDNO'];

			$sql = "INSERT INTO `tblmessage` (`MessageText`, `CLASSCODE`, `ACCOUNT_USERNAME`,FROM_USER,TO_USER,`COURSE`, `IDNO`) 
					VALUES ('{$messagetext}','{$classcode}','{$student}','{$course}','{$course}','{$student}','{$IDNO}')";
			$mydb->setQuery($sql);
	...
	function doEdit(){
		if(isset($_POST['save'])){
		 ...
		 //    redirect(web_root.'index.php?q=profile');
			...
			$stud->ACC_PASSWORD 		= sha1($_POST['PASS']); 
			$stud->update_account($_SESSION['IDNO']);
			...
			if ($image_size==FALSE ) {
				...
				redirect(web_root. "index.php?q=profile");
					...
					move_uploaded_file($temp,"student_image/" . $myfile);
						...
						$stud->updatephoto($_SESSION['IDNO']); 
...
function dodrop(){
 ...
 function doDeleteMessage(){
 	...
 	if (isset($_GET['id'] )) {
 		...
 		if (isset($cur)) {
 			# code...
 			message("Message already deleted!","success");
	 		redirect(web_root."index.php?q=profile");
 		}
 	}
